...
def splitFrames():
   ...
   oReader = csv.reader(open('/data2/nathan/embryo/originalLabels.csv', 'rU'), delimiter=',')
   oheader = oReader.next()
   print(oheader)
   odf = [row for row in oReader]
   odict = {}
   for row in odf:
      if row[wind] == '':
         break
      if row[pind] not in odict:
         odict[row[pind]] = {}
      odict[row[pind]][int(row[wind])] = row
   ...
   for f in os.listdir('/data2/nathan/embryo/videos'):
      ...
      if slide not in odict:
         continue
      print(f)
      wells = [int(well) for well in f.split('_wells_')[1].split('_')[:-1]]
      for i in range(len(wells)):
         if wells[i] not in odict[slide]:
            continue
         if slide in pdict and wells[i] in pdict[slide]:
            continue
         ...
         np.save(open('/data2/nathan/embryo/data/discardPatient' + str(slide) + 'well' + str(wells[i]) + 'feat.npy', 'wb'), X)
         np.save(open('/data2/nathan/embryo/data/discardPatient' + str(slide) + 'well' + str(wells[i]) + 'labelRaw.npy', 'wb'), Y)
